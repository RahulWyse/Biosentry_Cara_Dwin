package android.wyse.face.com.microchip.android.mcp2221comm;
...
import android.wyse.face.UsbLatchConnector;
...
public class Mcp2221Comm {
    ...
    private static Mcp2221Comm mcp2221Comm;
    public synchronized static Mcp2221Comm getInstance(){
        if (mcp2221Comm!=null){
            return mcp2221Comm;
        }else{
            return mcp2221Comm=new Mcp2221Comm(UsbLatchConnector.getInstance().getMcp2221());
        }
    }

    public static void setMcp2221Comm(Mcp2221Comm mcp2221Comm) {
        Mcp2221Comm.mcp2221Comm = mcp2221Comm;
    }
    ...
    public int readI2cData(final byte i2cAddress, final ByteBuffer i2cDataReceived,
        ...
        while (uiLclRxedLength < numberOfBytesToRead || numberOfBytesToRead == 0 && iLclTimeout > 0) {
            ...
            for (int i = 0; i < mRxData.get(3); i++) {
                // copy the user data
                i2cDataReceived.put(dataIndex++, mRxData.get(4 + i));
            }
